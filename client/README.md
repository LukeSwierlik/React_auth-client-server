...
- [Deployment](#deployment)
  ...
  - [Serving Apps with Client-Side Routing](#serving-apps-with-client-side-routing_link)
...
When you run `npm run build`, Create React App will substitute `%PUBLIC_URL%` with a correct absolute path so your project works even if you use client-side routing_link or host it at a non-root URL.
...
Create React App doesn't prescribe a specific routing_link solution, but [React Router](https://reacttraining.com/react-router/) is the most popular one.
...
Note that [you may need to configure your production server to support client-side routing_link](#serving-apps-with-client-side-routing_link) before deploying your app.
...
However this is not quite enough if you use client-side routing_link. Read the next section if you want to support URLs like `/todos/42` in your single-page app.
...
service worker navigation routing_link can be configured or disabled by
...
If you are not using the HTML5 `pushState` history API or not using client-side routing_link at all, it is unnecessary to specify the URL from which your app will be served. Instead, you can put this in your `package.json`:
...
**Support for client-side routing_link:**
...
Note that in order to support routers that use HTML5 `pushState` API, you may want to rename the `index.html` in your build folder to `200.html` before deploying to Surge. This [ensures that every URL falls back to that file](https://surge.sh/help/adding-a-200-page-for-client-side-routing_link).
